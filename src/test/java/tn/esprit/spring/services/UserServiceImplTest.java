...
class UserServiceImplMockTest {
    ...
    public void testAddUser() {
        ...
        assertEquals("John", result.getFirstName());  // Vérifie le prénom (prénom est "John")
        assertEquals("Doe", result.getLastName());  // Vérifie le nom (nom est "Doe")
    ...
    public void testRetrieveUser() {
        ...
        // Simulation du comportement du repository
        when(userRepository.findById(anyLong())).thenReturn(Optional.of(user));  // Mock du findById

        // Appel de la méthode à tester
        User result = userService.retrieveUser("1");  // Stocker le résultat
        ...
        assertEquals(role, result.getRole());  // Vérifie le rôle
        assertEquals(dateNaissance, result.getDateNaissance());  // Vérifie la date de naissance
        verify(userRepository, times(1)).findById(anyLong());  // Vérifie que findById a été appelé une fois
    }
